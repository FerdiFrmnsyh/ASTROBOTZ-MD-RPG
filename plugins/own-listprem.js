let handler = async (m, { conn, args }) => {
  ...
  await conn.sendButton(m.chat, `${htki} *ᴘʀᴇᴍɪᴜᴍ* ${htka}
┌✦ *ᴍʏ ᴘʀᴇᴍɪᴜᴍ ᴛɪᴍᴇ:*
┊• *ɴᴀᴍᴇ:* ${conn.getName(m.sender)}
${prem ? `${clockString (premiumTime - new Date() * 1)}` : '┊• *ᴘʀᴇᴍɪᴜᴍᴛɪᴍᴇ:* ᴇxᴘɪʀᴇᴅ 🚫'}
┗━═┅═━––––––๑

•·–––––––––––––––––––––·•
${sortedP.slice(0, len).map(({ jid, name, premiumTime, registered }, i) => `\n\n┌✦ ${registered ? name : conn.getName(jid)}\n┊• wa.me/${jid.split`@`[0]}\n${premiumTime > 0 ? `${clockString (premiumTime - new Date() * 1)}` : '┊ *ᴇxᴘɪʀᴇᴅ 🚫*'}`).join`\n┗━═┅═━––––––๑`}
