...
//
	...
	let __timers = (new Date - pengguna.lastfishing)
        ...
        if (pengguna.stamina < 20) return m.reply(`sᴛᴀᴍɪɴᴀ/ᴅᴀʀᴀʜ ᴀɴᴅᴀ ᴛᴅᴋ ᴄᴜᴋᴜᴘ\nɪsɪ sᴛᴀᴍɪɴᴀ ᴅᴇɴɢᴀɴ*${usedPrefix}eat8`)
    if (pengguna.lastfishing > 10800000) throw m.reply(`ᴋᴀᴍᴜ ᴍᴀsɪʜ ᴋᴇʟᴇʟᴀʜᴀɴ\nʜᴀʀᴀᴘ ᴛᴜɴɢɢᴜ *${timers}* ʟᴀɢɪ`)
...
ᴋᴀᴍᴜ ʙɪsᴀ ᴍᴇᴍᴀsᴀᴋ ᴅᴀɴ ᴍᴇᴍᴀᴋᴀɴɴʏᴀ💉
ᴄᴏɴᴛᴏʜ:_
${ usedPrefix }cook`    
        setTimeout(() => {
        conn.reply(m.chat, mcng, m)
         if (psepick > 0 ) {
         	...
         	conn.reply(m.chat, `ᴋᴀᴍᴜ ᴍᴇɴᴅᴀᴘᴀᴛᴋᴀɴ 🎁ᴄʜᴇsᴛ ᴡᴇᴀᴘᴏɴs ᴇᴘɪᴄ ${psepick} ɪᴛᴇᴍ`, m)
//conn.sendButton( m.chat, `You Get 🎁chest weapons epic ${psepick} item`, wm, [[`again`, `.mancing`]], m)
         } 
        if  (psenjata > 0 ) {
        	pengguna.psenjata+= psenjata * 1
        conn.reply(m.chat, `ʏᴏᴜ ɢᴇᴛ 🎁 ᴄʜᴇsᴛ ᴡᴇᴀᴘᴏɴ ᴇᴘɪᴄ ${psepick} ɪᴛᴇᴍ`, m)
        	}
        }, 38000)

        setTimeout(() => {
                     conn.reply(m.chat, `${pemancing} sᴇᴘᴇʀᴛɪɴʏᴀ ᴅᴀᴘᴀᴛ sᴇsᴜᴀᴛᴜ.`, m)
                      }, 28000)
       setTimeout(() => {
                     conn.reply(m.chat, `${pemancing} ᴍᴇɴᴜɴɢɢᴜ`, m)
                      }, 8000)
       setTimeout(() => {
                     conn.reply(m.chat, `${pemancing} sᴇᴅᴀɴɢ ᴍᴇᴍᴀɴᴄɪɴɢ...`, m)
                      }, 0)
        ...
        pengguna.nila += nila * 1
         ...
         pengguna.kepiting += kepiting * 1
           ...
           } else return conn.reply(`*[❗] kamu tidak punya kail pancingan dan umpan 100 🎣*\n\nketik .buy pancingan 1 untuk buy\n\nketik .buy umpan 100 untuk buy`, m)


// conn.reply(`*[❗] kamu tidak punya kail pancingan dan umpan 100 🎣*\n\nketik .buy pancingan 1 untuk buy\n\nketik .buy umpan 100 untuk buy`, m)
           break
	          ...
	          case 'easy':
                                  ...
                                  if (pancingan == 0 || umpan == 0) return conn.reply(`*[❗] kamu tidak punya kail pancingan dan umpan 100 🎣*\n\nketik .buy pancingan 1 untuk buy\n\nketik .buy umpan 100 untuk buy`, m)
