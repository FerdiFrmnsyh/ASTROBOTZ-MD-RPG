    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    this.reply(m.chat, `[🚩] *𝚂𝚎𝚙𝚎𝚛𝚝𝚒𝚗𝚢𝚊 𝚊𝚗𝚍𝚊 𝚌𝚞𝚛𝚊𝚗𝚐,𝚖𝚎𝚗𝚐𝚐𝚞𝚗𝚊𝚔𝚊𝚗 𝚔𝚊𝚕𝚔𝚞𝚕𝚊𝚝𝚘𝚛*\n𝙱𝚞𝚢 𝙻𝚒𝚖𝚒𝚝 /buy limit\n\n𝙲𝚑𝚎𝚊𝚝 Limit /ngechit`, )
                else 
                m.exp += xp
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `[🚩] *𝙻𝚒𝚖𝚒𝚝 𝙰𝚗𝚍𝚊 𝙷𝚊𝚋𝚒𝚜, 𝚂𝚒𝚕𝚊𝚑𝚔𝚊𝚗 𝙱𝚎𝚕𝚒 𝙻𝚒𝚖𝚒𝚝\n\nBuy Limit /buy limit`,)
                    continue 
                }
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    return m.reply(`*🗂️ ᴘʟᴜɢɪɴs:* ${m.plugin}\n*👤 sᴇɴᴅᴇʀ:* ${m.sender}\n*💬 ᴄʜᴀᴛ:* ${m.chat}\n*💻 ᴄᴏᴍᴍᴀɴᴅ:* ${usedPrefix}${command} ${args.join(" ")}\n📄 *ᴇʀʀᴏʀ ʟᴏɢs:*\n\n${text}`.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
            ...
            if (chat.welcome) {
                ...
                let welc = 'SELAMAT DATANG'
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || '𝚂𝙴𝙻𝙰𝙼𝙰𝚃 𝙳𝙰𝚃𝙰𝙽𝙶, @user').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow') :
                            (chat.sBye || this.bye || conn.bye || '𝚂𝙴𝙻𝙰𝙼𝚃 𝚃𝙸𝙽𝙶𝙶𝙰𝙻 @user')).replace(/@user/g, '@' + user.split`@`[0])
...
this.sendFile(id, pp, 'pp.jpg', text, null, false, { mentions: [user] })
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '𝚂𝙴𝙻𝙰𝙼𝙰𝚃, @user ```𝚂𝙴𝙺𝙰𝚁𝙰𝙽𝙶 𝚃𝙴𝙻𝙰𝙷 𝙼𝙴𝙽𝙹𝙰𝙳𝙸 𝙰𝙳𝙼𝙸𝙽```')
        case 'demote':
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '𝙳𝙴𝚃𝙴𝙲𝚃 @user ```𝚂𝙴𝙺𝙰𝚁𝙰𝙽𝙶 𝙱𝚄𝙺𝙰𝙽 𝙰𝙳𝙼𝙸𝙽```')
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || '```𝙳𝙴𝚂𝙺𝚁𝙸𝙿𝚂𝙸 𝚃𝙴𝙻𝙰𝙷 𝙳𝙸𝙶𝙰𝙽𝚃𝙸 𝙼𝙴𝙽𝙹𝙰𝙳𝙸```\n@desc').replace('@desc', groupUpdate.desc)
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || '```𝙽𝙰𝙼𝙰 𝙶𝚁𝚄𝙿 𝚃𝙴𝙻𝙰𝙷 𝙳𝙸𝙶𝙰𝙽𝚃𝙸 𝙼𝙴𝙹𝙰𝙳𝙸```\n@subject').replace('@subject', groupUpdate.subject)
        if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || '```𝙵𝙾𝚃𝙾 𝙶𝚁𝚄𝙿 𝚃𝙴𝙻𝙰𝙷 𝙳𝙸𝙶𝙰𝙽𝚃𝙸 𝙼𝙴𝙽𝙹𝙰𝙳𝙸```').replace('@icon', groupUpdate.icon)
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || '```𝙻𝙸𝙽𝙺 𝙶𝚁𝚄𝙿 𝚃𝙴𝙻𝙰𝙷 𝙳𝙸 𝚁𝙴𝚂𝙴𝚃```\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
text: `🚩 𝐃𝐄𝐓𝐄𝐂𝐓𝐎𝐑 𝐎𝐍*@${participant.split`@`[0]}* 𝐓𝐞𝐥𝐚𝐡 𝐌𝐞𝐧𝐠𝐡𝐚𝐩𝐮𝐬 𝐏𝐞𝐬𝐚𝐧.`,
contextInfo: {
externalAdReply: {
title: v,
thumbnailUrl: "https://telegra.ph/file/255c577ec25f52be7fb3b.jpg",
sourceUrl: sgc,
mediaType: 1,
...
let msgg = {
    unreg: `*「 🚩 𝐃𝐀𝐅𝐓𝐀𝐑 」*\n\n📝 𝚂𝚒𝚕𝚊𝚑𝚔𝚊𝚗 𝙳𝚊𝚏𝚝𝚊𝚛 𝙺𝚎 𝙳𝚊𝚝𝚊𝚋𝚊𝚜𝚎 𝚝𝚎𝚛𝚕𝚎𝚋𝚒𝚑 𝚍𝚊𝚑𝚞𝚕𝚞 𝚞𝚗𝚝𝚞𝚔 𝚖𝚎𝚗𝚐𝚐𝚞𝚗𝚊𝚔𝚊𝚗 𝚋𝚘𝚝 𝚒𝚗𝚒 𝚕𝚎𝚋𝚒𝚑 𝚕𝚊𝚗𝚓𝚞𝚝. 𝙶𝚞𝚗𝚊𝚔𝚊𝚗 𝙿𝚎𝚛𝚒𝚗𝚝𝚊𝚑 𝙱𝚎𝚛𝚒𝚔𝚞𝚝:\n\n👉 .daftar namaAnda.umur\n👤 Contoh: .*daftar MANUSIA.18*\n\n`
}[type]
...
externalAdReply: {
title: v,
thumbnailUrl: "https://telegra.ph/file/88120afb2dcd5e71910bf.jpg",
sourceUrl: sgc,
mediaType: 1,
renderLargerThumbnail: true
}}}, { quoted: m})
...
global.logo = 'https://telegra.ph/file/887e4c0c829e098dc8fcc.jpg'
global.hyuri = ['https://telegra.ph/file/5d908f4a17515a15c6202.jpg']
global.thumb = 'https://telegra.ph/file/2e8d2ef0926e848626a43.jpg' 
global.imagebot = 'https://telegra.ph/file/887e4c0c829e098dc8fcc.jpg'
global.giflogo = 'https://telegra.ph/file/d99857d74715351bdcd6c.mp4'
global.zykomd = 'https://telegra.ph/file/255c577ec25f52be7fb3b.jpg'
global.thumbs = ['https://telegra.ph/file/887e4c0c829e098dc8fcc.jpg']
global.hwaifu = ['https://telegra.ph/file/887e4c0c829e098dc8fcc.jpg','https://telegra.ph/file/887e4c0c829e098dc8fcc.jpg','https://telegra.ph/file/887e4c0c829e098dc8fcc.jpg','https://telegra.ph/file/887e4c0c829e098dc8fcc.jpg','https://telegra.ph/file/887e4c0c829e098dc8fcc.jpg']
global.thumbnailUrl = [
